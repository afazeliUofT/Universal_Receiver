...
class UPAIRChannelEstimator(tf.keras.Model):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.head = tf.keras.layers.Conv2D(3 * self.num_rx_ant, kernel_size=1, padding="same")
    ...
    def estimate_with_ls(
        ...
        head = self.head(z)
        real_delta, imag_delta, err_delta = tf.split(head, 3, axis=-1)
        ...
        err_hat = btfnc_to_tensor7(tf.cast(err_hat_btfnc, tf.complex64))
        err_hat = tf.math.real(err_hat)
