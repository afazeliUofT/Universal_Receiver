...
def _paper_cfgres_cache_path(paths: dict[str, Path], cfg: dict[str, Any]) -> Path:
    cache_name = str(get_cfg(cfg, "baselines.paper_configured_reservoir.covariance_cache_name", "paper_cfgres_covariances.npz"))
    return paths["artifacts"] / cache_name


def _paper_cfgres_basis_path(paths: dict[str, Path], cfg: dict[str, Any]) -> Path:
    name = str(get_cfg(cfg, "baselines.paper_configured_reservoir.basis_artifact_name", "paper_cfgres_basis.npz"))
...
def build_paper_configured_reservoir_estimator(
    ...
    local_cfg = dict(get_cfg(cfg, "baselines.paper_configured_reservoir", {}) or {})
    set_cfg(paper_cfg, "baselines.covariance_estimation.cache_name", str(local_cfg.get("covariance_cache_name", "paper_cfgres_covariances.npz")))
    set_cfg(paper_cfg, "baselines.covariance_estimation.reuse_cache", bool(local_cfg.get("reuse_cache", True)))
    set_cfg(
        paper_cfg,
    ...
    basis_path = _paper_cfgres_basis_path(paths, cfg)
    estimator.save_basis_artifact(basis_path)
    return estimator, {
        "paper_cfgres_covariances": str(_paper_cfgres_cache_path(paths, cfg)),
        "paper_cfgres_basis": str(basis_path),
    }
